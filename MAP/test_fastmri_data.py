...
def apply_mask(kspace, mask_type="variable_density", seed=42, **kwargs):
    ...
    if mask_type == "bernoulli":
        mask = BernoulliMask(kwargs.get("prob", 0.85), seed=seed).generate(shape)
    elif mask_type == "centered_bernoulli":
        mask = CenteredBernoulliMask(
            kwargs.get("prob", 0.75), kwargs.get("center_frac", 0.25), seed=seed
        ).generate(shape)
    elif mask_type == "variable_density":
        mask = VariableDensityMask(
            kwargs.get("type", "gaussian"), kwargs.get("decay", 2), seed=seed
        ).generate(shape)
    else:
...
def map_reconstruction(y, mask, **map_params):
    ...
    result = map_estimator.subgradient_descent(y)
    return result, map_estimator


def plot_graphs(original, masked_ifft, map_reconstruct, target=None, map_estimator=None):
    ...
    if map_estimator is not None and hasattr(map_estimator, "loss_history"):
        plt.figure()
...
def run_pipeline(split, index, mask_type, map_params=None, mask_params=None):
    map_params = map_params or dict(
        sigma=0.95, lambda_=0.01, eps=1e-2, learning_rate=0.01, max_iter=100
    )
    ...
    result, map_estimator = (map_reconstruction(y, mask, **map_params))
    map_reconstruct = np.abs(result)
    masked_ifft = np.abs(np.fft.ifft2(y))

    plot_graphs(zero_filled, masked_ifft, map_reconstruct, target=target, map_estimator=map_estimator)


def main():
    allowed_map_params = {"sigma", "lambda_", "eps", "learning_rate", "max_iters"}
    allowed_mask_params = {"prob", "center_frac", "seed", "type", "decay"}
    ...
    mask_params = {"prob": 0.01, "center_frac": 0.2, "seed": 30}

    for k in map_params:
        if k not in allowed_map_params:
            raise ValueError(f"Unknown map_params key: '{k}'")

    for k in mask_params:
        if k not in allowed_mask_params:
            raise ValueError(f"Unknown mask_params key: '{k}'")

    run_pipeline(
        split="train",
        index=4689,
        mask_type="centered_bernoulli",
